...
var bot_token = '5766514588:AAFvFzi8H1k4rGZvNGhwdbiyGUZgeNjND7E'; //YOUR BOT TOKEN HERE
var bot_name = "FreePay_RBot" // Bot username here without @
var gateway = "C";
/* A = Cash_RewardPe
   B = Full2SMS
   C = Fast X Pay
...
//CONNECT TO MONGO
mongo.connect('mongodb+srv://royalayush:royalayush@cluster0.p77fj.mongodb.net/NodeJSBot?retryWrites=true&w=majority', {
...
bot.hears('/adminhelp', async (ctx) => {
  try {
    ...
    if (ctx.from.id == 1574227974 || ctx.from.id == 1574227974) {
...
bot.hears('/broadcast', async (ctx) => {
  if (ctx.from.id ==  1574227974 || ctx.from.id == 881752385) {
    ctx.replyWithMarkdown(
  ...
  try {
    ...
    if (toWith == 0) {
      ...
      if (gateway == "A") {
        ...
        axios.post("https://fastxpay.co/payments/api/walletpay/?number="+wallet+"&amount="+amount+"&comment="+comment+"&guid="+swg+"&orderid="+RandInt+"");
      } else if (gateway == "D") {
...
async function mustjoin(ctx) {
  try {
    ...
    ctx.reply(
      "<b>⛔ Must Join All Our Channel</b>\n\nhttps://youtu.be/AH68ISfiC1s\n" + final + "\n<b>✅ After Joining, Click On '🟢 Joined'</b>", { disable_web_page_preview: true, parse_mode: 'html', reply_markup:  { keyboard: [['🟢 Joined']], resize_keyboard: true 
        }
      }
    )
  } catch (error) {
